...
test "build Docker project and verify layer cache" {
    ...
    // GEase Backend
    if (std.Io.Dir.openFileAbsolute(io, "/home/life-mac-africa/Work/GEase/backend/Dockerfile", .{})) |file_gease| {
        defer file_gease.close(io);
        var read_buf: [2048]u8 = undefined;
        var reader = file_gease.reader(io, &read_buf);
        var content_buf: [4096]u8 = undefined;
        const n = try reader.interface.readSliceShort(&content_buf);
        const metrics = try builder.buildWithMetrics(content_buf[0..n], "/home/life-mac-africa/Work/GEase/backend", "gease-backend:latest");
        metrics.printFormatted("Work/GEase/backend");
    } else |_| {}
}
...
test "experiment on Moby Dockerfile.simple and full Dockerfile" {
    ...
    if (std.Io.Dir.openFileAbsolute(io, "/home/life-mac-africa/Work/moby/Dockerfile.simple", .{})) |file_simple| {
        ...
        var content_buf: [4096]u8 = undefined;
        const n = try reader.interface.readSliceShort(&content_buf);
        const metrics_simple = try builder.buildWithMetrics(content_buf[0..n], "/home/life-mac-africa/Work/moby", "moby:simple");
        metrics_simple.printFormatted("Work/moby (Dockerfile.simple)");
    } else |_| {}
    ...
    if (std.Io.Dir.openFileAbsolute(io, "/home/life-mac-africa/Work/moby/Dockerfile", .{})) |file_full| {
        ...
        const n = try reader.interface.readSliceShort(&content_buf);
        const metrics_full = try builder.buildWithMetrics(content_buf[0..n], "/home/life-mac-africa/Work/moby", "moby:full");
        metrics_full.printFormatted("Work/moby (Production Dockerfile 646 lines)");
    } else |_| {}
}
...
test "experiment on Kubernetes and TensorFlow Dockerfiles" {
    ...
    if (std.Io.Dir.openFileAbsolute(io, "/home/life-mac-africa/Work/kubernetes/test/images/agnhost/Dockerfile", .{})) |file_k8s| {
        ...
        const n = try reader.interface.readSliceShort(&content_buf);
        const metrics_k8s = try builder.buildWithMetrics(content_buf[0..n], "/home/life-mac-africa/Work/kubernetes/test/images/agnhost", "k8s-agnhost:latest");
        metrics_k8s.printFormatted("Work/kubernetes (test/images/agnhost)");
    } else |_| {}
    ...
    if (std.Io.Dir.openFileAbsolute(io, "/home/life-mac-africa/Work/tensorflow/tensorflow/tools/tf_sig_build_dockerfiles/Dockerfile", .{})) |file_tf| {
        ...
        var content_buf: [8192]u8 = undefined;
        const n = try reader.interface.readSliceShort(&content_buf);
        const metrics_tf = try builder.buildWithMetrics(content_buf[0..n], "/home/life-mac-africa/Work/tensorflow/tensorflow/tools/tf_sig_build_dockerfiles", "tensorflow-builder:latest");
        metrics_tf.printFormatted("Work/tensorflow (tf_sig_build_dockerfiles)");
    } else |_| {}
}
